...
public interface PersonRepository extends CrudRepository<Profile, Integer> {
    ...
    /**
     * Получаем список всех ProfileDTO по множеству id
     *
     * @param ids Set < Integer> Profile ids
     * @return List < ProfileDTO>
     */
    @Query("SELECT new ru.checkdev.auth.dto.ProfileDTO"
           + "(p.id, p.username, p.experience, p.photo.id, p.updated, p.created)"
           + " FROM profile p WHERE p.id in(:ids)")
    List<ProfileDTO> findAllById(Set<Integer> ids);
}
